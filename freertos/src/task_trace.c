...
/* ── Direct Buffer Access Functions ───────────────────────────────────── */
...
const task_trace_t* trace_get_buffer(uint32_t *out_count, uint32_t *out_start_idx) {
    ...
    if (out_start_idx) *out_start_idx = start;
...
bool trace_get_entry(uint32_t logical_idx, task_trace_t *out_entry) {
    ...
    if (logical_idx >= count || !out_entry) {
        return false;
    }
    
    uint32_t idx = (start + logical_idx) % TRACE_BUFFER_SIZE;
    *out_entry = trace_buf[idx];
    return true;
}
...
void trace_calculate_stats(trace_task_id_t task_id, task_stats_t *out_stats) {
    ...
    // Initialize stats
    out_stats->count = 0;
    out_stats->min_exec_us = UINT32_MAX;
    out_stats->max_exec_us = 0;
    out_stats->avg_exec_us = 0;
    out_stats->min_response_us = UINT32_MAX;
    out_stats->max_response_us = 0;
    out_stats->avg_response_us = 0;
    
    uint32_t count, start;
    trace_get_buffer(&count, &start);
    
    uint64_t total_exec_us = 0;
    uint64_t total_response_us = 0;
    ...
    for (uint32_t i = 0; i < count; i++) {
        ...
        if (exec_us < out_stats->min_exec_us) out_stats->min_exec_us = exec_us;
        if (exec_us > out_stats->max_exec_us) out_stats->max_exec_us = exec_us;
        total_exec_us += exec_us;
        
        // Response time stats
        if (resp_us < out_stats->min_response_us) out_stats->min_response_us = resp_us;
        if (resp_us > out_stats->max_response_us) out_stats->max_response_us = resp_us;
        total_response_us += resp_us;
    }
    
    if (out_stats->count > 0) {
        out_stats->avg_exec_us = (uint32_t)(total_exec_us / out_stats->count);
        out_stats->avg_response_us = (uint32_t)(total_response_us / out_stats->count);
    } else {
        out_stats->min_exec_us = 0;
        out_stats->min_response_us = 0;
    }
}
